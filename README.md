...
- 白背景、blue-600アクセント、slate系テキストを基本にしたシンプルなホテル探索UIです。
- 共通ヘッダーには `Travel Reserve`、ホテルを探す、お気に入り、価格推移、このサイトについての導線を配置しています。
- 上部に横長の検索バーを配置し、目的地、最低価格、最高価格を指定できます。利用者評価、ホテルクラス、設備条件は左サイドフィルターで指定します。
...
- PCでは左にstickyなフィルターサイドバー、右にホテル一覧を置く2カラム構成です。スマホでは「絞り込みを開く」ボタンでフィルターを開閉します。
- ホテルカードは横長カードとして、画像、ホテル名、評価、レビュー数、住所・アクセス、特徴タグ、参考最安値、詳細、楽天トラベル確認導線を整理しています。
- ページ下部には、お気に入り追加、価格推移追跡、予約タイミング確認の価値訴求カードを表示します。
- お気に入りページとホテル詳細ページも同じ白背景カード、blue-600アクセント、控えめな影のトーンに統一しています。
...
- ホテル取得は完全無制限ではありません。楽天APIの呼び出し回数、Vercelの実行時間、429 rate_limitedを避けるため、検索条件に応じて取得件数の上限を決めます。
- 条件なしトップ画面では `/api/hotels` を呼ばず、ホテル一覧も取得しません。APIを直接呼んだ場合も `searchSkipped=true` と空配列を返します。
- 目的地ありの検索では、楽天APIの `hits=30` を前提に最大120件、つまり最大4ページ分を取得します。`RAKUTEN_SEARCH_MAX_HOTELS_DEFAULT` で調整できます。
- 目的地に加えて価格、評価、ホテルクラス、設備条件のいずれかがある場合は、最大180件、つまり最大6ページ分まで候補取得を広げます。`RAKUTEN_SEARCH_MAX_HOTELS_WITH_FILTERS` で調整できます。
- どの条件でも1回のトップ検索の絶対上限は300件です。`RAKUTEN_SEARCH_MAX_HOTELS_HARD_LIMIT` で下げられますが、トップ画面から無制限取得はしません。
- 同一検索条件は短時間メモリキャッシュします。楽天検索キャッシュは `rakuten-search:{criteria}:maxHotels:{maxHotels}` 相当のキーで、TTLは約15分です。
- 複数ページ取得中に楽天APIが429を返した場合は、それ以降の取得を止め、取得済み候補だけ表示します。debugでは `rateLimitHit=true` と取得済みページ数を確認できます。
